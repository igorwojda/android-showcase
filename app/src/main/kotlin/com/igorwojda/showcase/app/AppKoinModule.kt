...
val appModule =
    module {
         ...
         * - Lazy initialization reduces startup overhead by deferring setup until first network request.
         */
        single {
            // Use lazy delegate to defer interceptor creation until first network call
            val lazyInterceptor = lazy {
                HttpLoggingInterceptor { message ->
                    Timber.tag(TimberLogTags.NETWORK).d(message)
                }.apply {
                    /*
